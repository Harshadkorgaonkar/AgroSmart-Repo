...
class WeederPage extends StatefulWidget {
  const WeederPage({super.key});

  @override
  State<WeederPage> createState() => _WeederPageState();
}

class _WeederPageState extends State<WeederPage> {
  final supabase = Supabase.instance.client;

  List<Map<String, dynamic>> weeders = [];
  bool isLoading = true;
  String errorMessage = '';
  ...
  Future<void> fetchWeeders() async {
    setState(() {
      isLoading = true;
      errorMessage = '';
    });
    ...
    try {
      final response = await supabase
          ...
          .eq('e_type', 'Power Weeder')
          .eq('e_hide', false);

      weeders = (response as List<dynamic>).map<Map<String, dynamic>>((item) {
  ...
  Future<void> _addToCart(Map<String, dynamic> item) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '✅ Added ${item['model'] ?? item['e_model'] ?? 'item'} to cart',
          ),
  ...
  List<Map<String, dynamic>> get filteredWeeders {
    return weeders.where((item) {
