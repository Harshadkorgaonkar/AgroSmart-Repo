...
class CultivatorPage extends StatefulWidget {
  const CultivatorPage({super.key});

  @override
  State<CultivatorPage> createState() => _CultivatorPageState();
}

class _CultivatorPageState extends State<CultivatorPage> {
  final supabase = Supabase.instance.client;

  List<Map<String, dynamic>> cultivators = [];
  bool isLoading = true;
  String errorMessage = '';
  ...
  Future<void> fetchCultivators() async {
    setState(() {
      isLoading = true;
      errorMessage = '';
    });
    ...
    try {
      final response = await supabase
          ...
          .eq('e_type', 'Cultivator')
          .eq('e_hide', false);

      cultivators = (response as List<dynamic>).map<Map<String, dynamic>>((
        item,
      ) {
  ...
  List<Map<String, dynamic>> get filteredCultivators {
    return cultivators.where((item) {
